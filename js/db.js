...
const DB = (() => {
  ...
  const STUDENTS = {
    ...
    set:          (id,d)     => set(`students/${id.replace(/[^a-zA-Z0-9]/g,'_')}`,d),
    update:       (id,d)     => update(`students/${id.replace(/[^a-zA-Z0-9]/g,'_')}`,d),
    byEmail:      async e    => { const a=await arr('students');return a.find(s=>s.email===e)||null; },
    byStudentId:  async id   => { const a=await arr('students');return a.find(s=>s.studentId.toUpperCase()===id.toUpperCase())||null; },
    addDevice:    (id,fp)    => set(`students/${id.replace(/[^a-zA-Z0-9]/g,'_')}/devices/${k(fp)}`,Date.now()),
    hasDevice:    async(id,fp)=>!!(await get(`students/${id.replace(/[^a-zA-Z0-9]/g,'_')}/devices/${k(fp)}`)),
  };
