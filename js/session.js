...
const LEC = (() => {
  ...
  function getLoc() { const btn = UI.Q('get-loc-btn'), res = UI.Q('loc-result'); btn.disabled = true; btn.innerHTML = '<span class="spin"></span>Getting location…'; res.className = 'loc-result'; res.innerHTML = '<div class="loc-dot pulsing"></div> Acquiring GPS…'; if (!navigator.geolocation) { _demoLoc(); return; } navigator.geolocation.getCurrentPosition(p => { S.lecLat = p.coords.latitude; S.lecLng = p.coords.longitude; S.locAcquired = true; _locOK(p.coords.accuracy); }, () => _demoLoc(), { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 }); }
  ...
  async function endSession() { const ok = await MODAL.confirm('End session?', 'All records will be saved.', { icon: '🛑', confirmLabel: 'End session', confirmCls: 'btn-danger' }); if (!ok) return; stopTimers(); await _markEnded('manual', 'Manually ended'); resetForm(); await MODAL.success('Session ended', 'All records saved.'); }
  function stopTimers() { clearInterval(S.tickTimer); S.tickTimer = null; if (S.unsubRec) { S.unsubRec(); S.unsubRec = null; } if (S.unsubBlk) { S.unsubBlk(); S.unsubBlk = null; } _stopHeartbeat(); }
  ...
  async function exportLiveCSV() { if (!S.session) return; const recs = await DB.SESSION.getRecords(S.session.id); const rows = [['#', 'Name', 'Student ID', 'Biometric ID', 'Location', 'Time', 'Course', 'Lecturer', 'Date']]; recs.forEach((r, i) => rows.push([i + 1, r.name, r.studentId, (r.biometricId || '').slice(0, 16), r.locNote || '', r.time, S.session.courseCode, S.session.lecturer, S.session.date])); UI.dlCSV(rows, `ATT_${S.session.courseCode}_LIVE`); }

  async function _loadRecords() { const el = UI.Q('records-list'); el.innerHTML = '<div class="att-empty">Loading…</div>'; try { const user = AUTH.getSession(); const myLecId = user?.role === 'ta' ? (user?.activeLecturerId || user?.id || '') : (user?.id || ''); const sessions = (await DB.SESSION.byLec(myLecId)).filter(s => !s.deletedByLec).sort((a, b) => b.createdAt - a.createdAt); if (!sessions.length) { el.innerHTML = '<div class="no-rec">No sessions yet.</div>'; return; } el.innerHTML = sessions.map(s => { const recs = s.records ? Object.values(s.records) : []; const isActive = s.active === true; const statusBadge = isActive ? `<span class="pill pill-teal" style="background:var(--teal);color:white">🟢 ACTIVE</span>` : `<span class="pill pill-gray">🔴 ENDED</span>`; const endButton = isActive ? `<button class="btn btn-warning btn-sm" onclick="LEC.endSessionFromRecord('${s.id}')" style="background:var(--amber)">⏹️ End Session</button>` : ''; return `<div class="sess-card" style="border-left:4px solid ${isActive ? 'var(--teal)' : 'var(--text4)'}"><div class="sc-hdr"><div><div class="sc-title">${UI.esc(s.courseCode)} — ${UI.esc(s.courseName)} ${statusBadge}</div><div class="sc-meta">${UI.esc(s.date)} · ${recs.length} present · Duration: ${UI.fmtDur(s.durationMins || 60)}</div>${isActive ? `<div class="sc-meta" style="color:var(--teal);font-size:11px">⏱️ Expires: ${new Date(s.expiresAt).toLocaleTimeString()}</div>` : ''}</div><div class="sc-actions">${endButton}<button class="btn btn-secondary btn-sm" onclick="LEC.exportSessCSV('${s.id}')">⬇ CSV</button>${!isActive ? `<button class="btn btn-danger btn-sm" onclick="LEC.deleteSess('${s.id}')">🗑️ Del</button>` : ''}</div></div>${recs.length ? `<div style="margin-top:10px">${recs.slice(0, 5).map((r, i) => `<div class="rec-row"><span style="font-size:11px;min-width:22px">${i + 1}.</span><span class="rec-name">${UI.esc(r.name)}</span><span class="rec-sid">${UI.esc(r.studentId)}</span><span class="rec-time">${UI.esc(r.time)}</span></div>`).join('')}${recs.length > 5 ? `<div style="font-size:11px;padding:4px 0">…${recs.length - 5} more</div>` : ''}</div>` : '<div class="no-rec" style="margin-top:8px">No check-ins yet</div>'}</div>`; }).join(''); } catch (err) { el.innerHTML = `<div class="no-rec">Error: ${UI.esc(err.message)}</div>`; } }
  async function endSessionFromRecord(sessionId) { const session = await DB.SESSION.get(sessionId); if (!session) { await MODAL.error('Not found', 'Session not found.'); return; } if (!session.active) { await MODAL.alert('Already ended', 'Session already ended.'); _loadRecords(); return; } const confirm = await MODAL.confirm('End this session?', `<strong>${UI.esc(session.courseCode)} — ${UI.esc(session.courseName)}</strong><br/>Date: ${session.date}<br/>Check-ins: ${session.records ? Object.keys(session.records).length : 0}`, { icon: '🛑', confirmLabel: 'End Session', confirmCls: 'btn-warning' }); if (!confirm) return; try { const recs = await DB.SESSION.getRecords(sessionId), blks = await DB.SESSION.getBlocked(sessionId); await DB.SESSION.update(sessionId, { active: false, endedAt: Date.now(), manuallyEnded: true, endedBy: 'manual_from_records' }); await DB.BACKUP.save(session.lecFbId || session.lecId, sessionId, { session: { ...session, active: false }, records: recs, blocked: blks, savedAt: new Date().toISOString() }); if (S.session && S.session.id === sessionId) { stopTimers(); S.session = null; resetForm(); } await MODAL.success('Session Ended', `Session for ${session.courseCode} ended. ${recs.length} check-in${recs.length !== 1 ? 's were' : ' was'} saved.`); _loadRecords(); if (document.querySelector('#view-lecturer .tab.active')?.dataset?.tab === 'session' && S.session === null) resetForm(); } catch (err) { await MODAL.error('Error', err.message); } }
  async function deleteSess(id) { const session = await DB.SESSION.get(id); if (session && session.active) { const endFirst = await MODAL.confirm('Cannot delete active session', 'End it first?', { confirmLabel: 'Yes, end it', cancelLabel: 'Cancel', confirmCls: 'btn-warning' }); if (endFirst) await endSessionFromRecord(id); return; } const ok = await MODAL.confirm('Delete session?', 'Admin backup preserved.', { confirmCls: 'btn-danger' }); if (!ok) return; try { await DB.SESSION.update(id, { deletedByLec: true }); _loadRecords(); } catch (err) { MODAL.error('Error', err.message); } }
  async function exportSessCSV(id) { const s = await DB.SESSION.get(id); if (!s) return; const recs = await DB.SESSION.getRecords(id); if (!recs.length) { MODAL.alert('No records', 'No check-ins to export.'); return; } const rows = [['#', 'Name', 'Student ID', 'Biometric ID', 'Location', 'Time', 'Course', 'Lecturer', 'Date']]; recs.forEach((r, i) => rows.push([i + 1, r.name, r.studentId, (r.biometricId || '').slice(0, 16), r.locNote || '', r.time, s.courseCode, s.lecturer, s.date])); UI.dlCSV(rows, `ATT_${s.courseCode}_${s.date}`); }

  async function _loadReports() { const el = UI.Q('reports-list'); el.innerHTML = '<div class="att-empty">Loading…</div>'; try { const user = AUTH.getSession(); const all = (await DB.SESSION.byLec(user?.role === 'ta' ? (user?.activeLecturerId || user?.id || '') : (user?.id || ''))).filter(s => !s.deletedByLec); if (!all.length) { el.innerHTML = '<div class="no-rec">No sessions yet.</div>'; return; } const grouped = {}; all.forEach(s => { if (!grouped[s.courseCode]) grouped[s.courseCode] = { code: s.courseCode, name: s.courseName, sessions: [] }; grouped[s.courseCode].sessions.push(s); }); el.innerHTML = Object.values(grouped).map(g => { const total = g.sessions.reduce((n, s) => n + (s.records ? Object.keys(s.records).length : 0), 0); return `<div class="sess-card"><div class="sc-hdr"><div><div class="sc-title">${UI.esc(g.code)} — ${UI.esc(g.name)}</div><div class="sc-meta">${g.sessions.length} session${g.sessions.length !== 1 ? 's' : ''} · ${total} total check-ins</div></div><div class="sc-actions"><button class="btn btn-ug btn-sm" onclick="LEC.exportCourseXL('${UI.esc(g.code)}')">⬇ Excel</button><button class="btn btn-secondary btn-sm" onclick="LEC.exportCourseCSV('${UI.esc(g.code)}')">⬇ CSV</button></div></div></div>`; }).join(''); } catch (err) { el.innerHTML = `<div class="no-rec">Error: ${UI.esc(err.message)}</div>`; } }
  ...
  async function _loadTAs() { const el = UI.Q('ta-list'); if (!el) return; el.innerHTML = '<div class="att-empty">Loading…</div>'; try { const user = AUTH.getSession(), myId = user?.id || ''; const all = await DB.TA.getAll(), mine = all.filter(ta => ta.lecturers?.includes(myId)); if (UI.Q('ta-count')) UI.Q('ta-count').textContent = mine.length; if (!mine.length) { el.innerHTML = '<div class="no-rec">No TAs yet.</div>'; return; } el.innerHTML = mine.map(ta => `<div class="att-item"><div class="att-dot" style="background:var(--teal)"></div><span class="att-name">${UI.esc(ta.name || '(not registered)')}</span><span class="att-sid">${UI.esc(ta.email)}</span><span class="pill ${ta.status === 'active' ? 'pill-teal' : 'pill-gray'}">${ta.status === 'active' ? '✓ Active' : 'Pending'}</span><button class="btn btn-danger btn-sm" style="margin-left:auto" onclick="LEC.removeTA('${ta.id}','${UI.esc(ta.name || ta.email)}')">Remove</button></div>`).join(''); } catch (err) { el.innerHTML = `<div class="no-rec">Error: ${UI.esc(err.message)}</div>`; } }
