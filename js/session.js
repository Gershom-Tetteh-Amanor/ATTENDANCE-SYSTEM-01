/* session.js — Lecturer & TA dashboard
   ...
   Course Management: End/reactivate courses for semesters.
*/
'use strict';
...
const LEC = (() => {
  ...
  function _renderAtt(records) {
    ...
    UI.Q('l-att-list').innerHTML=records.length
      ? records.map((r,i)=>`<div class="att-item"><div class="att-dot"></div><span style="font-size:11px;color:var(--text4);min-width:22px">${i+1}.</span><span class="att-name">${UI.esc(r.name)}</span><span class="att-sid">${UI.esc(r.studentId)}</span><span class="pill pill-gray">🔏 ${UI.esc((r.biometricId||'').slice(0,8))}</span>${r.locNote?`<span class="pill pill-teal">📍 ${UI.esc(r.locNote)}</span>`:''}<span class="att-time">${UI.esc(r.time)}</span></div>`).join('')
  ...
  async function _markEnded(endedBy = 'manual', reason = '') {
    ...
    try {
      ...
      await DB.BACKUP.save(S.session.lecId||S.session.lecFbId,S.session.id,{
        ...
        records:recs, blocked:blks, savedAt:new Date().toISOString()
      });
  ...
  async function endSession() {
    ...
    if (!ok) return;
    stopTimers(); await _markEnded('manual', 'Lecturer/TA manually ended session'); resetForm(); await MODAL.success('Session ended','All records saved.');
  }
  ...
  async function _loadRecords() {
    ...
    try {
      ...
      el.innerHTML=sessions.map(s=>{
        ...
        let statusBadge = isActive ? `<span class="pill pill-teal" style="background:var(--teal);color:white">🟢 ACTIVE</span>` : `<span class="pill pill-gray">🔴 ENDED</span>`;
        let endButton = isActive ? `<button class="btn btn-warning btn-sm" onclick="LEC.endSessionFromRecord('${s.id}')" style="background:var(--amber);color:var(--text1)">⏹️ End Session</button>` : '';
        return `<div class="sess-card" style="border-left:4px solid ${isActive ? 'var(--teal)' : 'var(--text4)'}"><div class="sc-hdr"><div><div class="sc-title">${UI.esc(s.courseCode)} — ${UI.esc(s.courseName)} ${statusBadge}</div><div class="sc-meta">${UI.esc(s.date)} · ${recs.length} present · Duration: ${UI.fmtDur(s.durationMins||60)}</div>${isActive ? `<div class="sc-meta" style="color:var(--teal);font-size:11px">⏱️ Expires: ${new Date(s.expiresAt).toLocaleTimeString()}</div>` : ''}</div><div class="sc-actions">${endButton}<button class="btn btn-secondary btn-sm" onclick="LEC.exportSessCSV('${s.id}')">⬇ CSV</button>${!isActive ? `<button class="btn btn-danger btn-sm" onclick="LEC.deleteSess('${s.id}')">🗑️ Del</button>` : ''}</div></div>${recs.length ? `<div style="margin-top:10px">${recs.slice(0,5).map((r,i)=>`<div class="rec-row"><span style="font-size:11px;color:var(--text4);min-width:22px">${i+1}.</span><span class="rec-name">${UI.esc(r.name)}</span><span class="rec-sid">${UI.esc(r.studentId)}</span><span class="rec-time">${UI.esc(r.time)}</span></div>`).join('')}${recs.length>5?`<div style="font-size:11px;color:var(--text4);padding:4px 0">…${recs.length-5} more</div>`:''}</div>` : '<div class="no-rec" style="margin-top:8px">No check-ins yet</div>'}</div>`;
      }).join('');
  ...
  async function endSessionFromRecord(sessionId) {
    ...
    if (!session) { await MODAL.error('Not found', 'This session no longer exists.'); return; }
    if (!session.active) { await MODAL.alert('Already ended', 'This session has already been ended.'); _loadRecords(); return; }
    ...
    try {
      ...
      await DB.BACKUP.save(session.lecFbId || session.lecId, sessionId, { session: { ...session, active: false, manuallyEnded: true }, records: recs, blocked: blks, savedAt: new Date().toISOString() });
      if (S.session && S.session.id === sessionId) { stopTimers(); S.session = null; resetForm(); }
      ...
      _loadRecords();
      if (document.querySelector('#view-lecturer .tab.active')?.dataset?.tab === 'session' && S.session === null) resetForm();
  ...
  async function _loadReports() {
    ...
    try {
      ...
      el.innerHTML=Object.values(grouped).map(g=>{const total=g.sessions.reduce((n,s)=>n+(s.records?Object.keys(s.records).length:0),0);
  ...
  async function exportCourseXL(code) {
    if (typeof XLSX === 'undefined'){MODAL.alert('Library not ready','SheetJS not loaded yet.');return;}
  ...
  async function _loadCourses() {
    ...
    try {
      ...
      for (const session of sessions) {
        const normCode = DB.normalizeCourseCode(session.courseCode);
        if (!uniqueCourses[normCode]) {
          const courseRecord = await DB.COURSE.get(session.courseCode);
          uniqueCourses[normCode] = {
            code: session.courseCode,
            name: session.courseName,
            active: courseRecord ? courseRecord.active : true,
            year: courseRecord ? courseRecord.year : currentPeriod.year,
            semester: courseRecord ? courseRecord.semester : currentPeriod.semester,
            lastSessionDate: session.date
          };
        }
      }
      const activeCourses = Object.values(uniqueCourses).filter(c => c.active === true);
      const inactiveCourses = Object.values(uniqueCourses).filter(c => c.active === false);
      activeEl.innerHTML = activeCourses.length ? activeCourses.map(c => `<div class="course-management-card"><div class="course-header"><div class="course-code">${UI.esc(c.code)}</div><div class="course-status active">🟢 Active</div></div><div class="course-name">${UI.esc(c.name)}</div><div class="course-meta">Year: ${c.year} | Semester: ${c.semester === 1 ? 'First' : 'Second'}</div><button class="btn btn-warning btn-sm" onclick="LEC.endCourseForSemester('${c.code}')">⏹️ End Course for Semester</button></div>`).join('') : '<div class="no-rec">No active courses this semester.</div>';
      if (historyEl) {
        historyEl.innerHTML = inactiveCourses.length ? inactiveCourses.map(c => `<div class="course-management-card archived"><div class="course-header"><div class="course-code">${UI.esc(c.code)}</div><div class="course-status inactive">🔴 Archived</div></div><div class="course-name">${UI.esc(c.name)}</div><div class="course-meta">Last active: ${c.lastSessionDate}</div><button class="btn btn-teal btn-sm" onclick="LEC.reactivateCourse('${c.code}')">🔄 Reactivate for Next Semester</button></div>`).join('') : '<div class="no-rec">No archived courses.</div>';
      }
    } catch(err) { console.error('Load courses error:', err); if (activeEl) activeEl.innerHTML = `<div class="no-rec">Error: ${UI.esc(err.message)}</div>`; }
  }

  async function endCourseForSemester(courseCode) {
    const confirm = await MODAL.confirm('End Course for Semester', `End <strong>${UI.esc(courseCode)}</strong> for this semester? Students cannot check in until reactivated.`, { confirmLabel: 'Yes, End Course', confirmCls: 'btn-warning' });
    if (!confirm) return;
    try {
      const user = AUTH.getSession();
      await DB.COURSE.endCourseForSemester(courseCode, user?.id);
      await MODAL.success('Course Ended', `${courseCode} has been ended for the semester.`);
      _loadCourses();
    } catch(err) { await MODAL.error('Error', err.message); }
  }
  ...
  async function reactivateCourse(courseCode) {
    const currentPeriod = DB.getCurrentAcademicPeriod();
    const confirm = await MODAL.confirm('Reactivate Course', `Reactivate <strong>${UI.esc(courseCode)}</strong> for Year ${currentPeriod.year} - Semester ${currentPeriod.semester === 1 ? 'First' : 'Second'}?`, { confirmLabel: 'Yes, Reactivate', confirmCls: 'btn-teal' });
    if (!confirm) return;
    try {
      const user = AUTH.getSession();
      await DB.COURSE.reactivateCourse(courseCode, currentPeriod.year, currentPeriod.semester, user?.id);
      await MODAL.success('Course Reactivated', `${courseCode} is now active for the current semester.`);
      _loadCourses();
    } catch(err) { await MODAL.error('Error', err.message); }
  }
  ...
  async function inviteTA() {
    const emailEl = UI.Q('ta-email-input'), nameEl = UI.Q('ta-name-input'), courseEl = UI.Q('ta-course-input');
    const email = (emailEl?.value || '').trim().toLowerCase(), taName = nameEl?.value?.trim() || '', courseName = courseEl?.value?.trim() || 'Course';
    ...
    try {
      const user = AUTH.getSession(), myId = user?.id || '', myName = user?.name || 'Your Lecturer';
      const existing = await DB.TA.byEmail(email);
      if (existing) {
        const lecs = existing.lecturers || [];
        if (lecs.includes(myId)) { UI.btnLoad('ta-invite-btn', false, 'Send invite'); return UI.setAlert('ta-add-alert', '⚠️ This TA is already linked.'); }
        await DB.TA.update(existing.id, { lecturers: [...lecs, myId] });
        emailEl.value = ''; if(nameEl) nameEl.value = ''; if(courseEl) courseEl.value = '';
        UI.btnLoad('ta-invite-btn', false, 'Send invite');
        await MODAL.success('TA Linked!', `${taName} (${email}) added to your dashboard.`);
        _loadTAs(); return;
      }
      const code = UI.makeCode(), invKey = UI.makeToken(), signupLink = `${CONFIG.SITE_URL}?code=${code}#ta-signup`;
      await DB.TA.setInvite(invKey, { code, toEmail: email, toName: taName, lecturerId: myId, lecturerName: myName, courseName, createdAt: Date.now(), expiresAt: Date.now() + (48 * 3600 * 1000), usedAt: null });
      emailEl.value = ''; if(nameEl) nameEl.value = ''; if(courseEl) courseEl.value = '';
      UI.btnLoad('ta-invite-btn', false, 'Send invite');
      await MODAL.alert('Invite Code Generated', `<div style="text-align:center"><div style="margin-bottom:16px">Share with <strong>${UI.esc(taName)}</strong> at <strong>${UI.esc(email)}</strong></div><div style="background:var(--ug);color:var(--gold);padding:20px;border-radius:12px;margin-bottom:16px"><div style="font-size:12px">6-Character Invite Code</div><div style="font-family:monospace;font-size:36px;font-weight:700;letter-spacing:4px">${code}</div><div style="font-size:11px">Valid for 48 hours</div></div><div style="display:flex;gap:10px;justify-content:center"><button class="btn btn-secondary btn-sm" onclick="navigator.clipboard.writeText('${code}')">📋 Copy Code</button><button class="btn btn-secondary btn-sm" onclick="navigator.clipboard.writeText('${signupLink}')">🔗 Copy Link</button></div></div>`, { icon: '🎓', btnLabel: 'Done' });
      _loadTAs();
