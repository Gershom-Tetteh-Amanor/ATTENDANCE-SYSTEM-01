/* auth.js — Authentication for all roles (COMPLETE FIXED VERSION) */
'use strict';
...
const AUTH = (() => {
  ...
  async function adminLogin() {
    ...
    try {
      ...
      if(ca) {
        ...
        UI.btnLoad('al-btn',false,'Sign in');
        console.log('[AUTH] Co-admin login successful');
        if (typeof APP !== 'undefined' && APP.activateAdmin) {
  ...
  /* ══ LECTURER LOGIN (FIXED) ══ */
  async function lecLogin() {
    console.log('[AUTH] lecLogin called');
  ...
  async function taLogin() {
    console.log('[AUTH] taLogin called');
    ...
    try {
      ...
      if (activeLecturers.length === 1) {
        ...
        UI.btnLoad('tl-btn', false, 'Sign in');
        console.log('[AUTH] TA login successful');
        if (typeof APP !== 'undefined' && APP.activateLecturer) {
        ...
        sessionStorage.setItem('active_lecturer_id', selected.id);
        UI.btnLoad('tl-btn', false, 'Sign in');
        console.log('[AUTH] TA login successful with lecturer selection');
        if (typeof APP !== 'undefined' && APP.activateLecturer) {
          await APP.activateLecturer({ ...ta, role: 'ta', activeLecturerId: selected.id });
        }
      }
  ...
  async function studentLogin() {
    console.log('[AUTH] studentLogin called');
    ...
    try {
      ...
      UI.btnLoad('sl-btn', false, 'Sign in');
      console.log('[AUTH] Student login successful');
  ...
  async function _selectLecturerModal(lecturers) {
    return new Promise((resolve) => {
      const options = lecturers.map((l, i) => `
        <div onclick="window._selectLecturerCallback(${i})" style="
          padding:15px; 
  ...
  async function testAuth() {
    ...
    console.log('[AUTH] Available functions:', Object.keys(AUTH));
    return true;
  }
...
console.log('[AUTH] Auth module loaded. Available functions:', Object.keys(AUTH));
