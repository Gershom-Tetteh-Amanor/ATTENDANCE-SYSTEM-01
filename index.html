...
const LEC = (() => {
  ...
  async function _loadRecords() { 
    ...
    try { 
      ...
      if (!sessions.length) { 
        el.innerHTML = '<div class="no-rec">No sessions yet.</div>'; 
        return; 
      } 
  ...
  async function exportCourseXL(code) { 
    ...
    try { 
      ...
      if (!all.length) { 
        MODAL.close(); 
        MODAL.alert('No data', 'No sessions for this course.'); 
        return; 
      } 
